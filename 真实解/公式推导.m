...
%% 1.1
ux(x, y, t) = -exp(t) * x^2 * (x - 1)^2 * y * (y - 1) * (2 * y - 1) / 256;
uy(x, y, t) = exp(t) * x * (x - 1) * (2 * x - 1) * y^2 * (y - 1)^2/256;
p(x, y, t) = exp(t) * (x^3 - 1/4);

%% 1.2
ux(x, y, t) = exp(t) * sin(pi * x)^2 * sin(2 * pi * y);
uy(x, y, t) = -exp(t) * sin(2 * pi * x) * sin(pi * y)^2;
p(x, y, t) = exp(t) * (sin(pi * y) - 2 / pi);

%% 1.3
ux(x, y, t) = 20 * x^2 * (x - 1)^2 * y * (y - 1) * (2 * y - 1) * t;
uy(x, y, t) = -20 * x * (x - 1) * (2 * x - 1) * y^2 * (y - 1)^2 * t;
p(x, y, t) = 10 * (2 * x - 1) * (2 * y - 1);

%% 1.4
%% 压强空间收敛阶不稳定
ux(x, y, t) = 2 * pi * sin(pi * x)^2 * sin(pi * y) * cos(pi * y) * cos(t);
uy(x, y, t) = -2 * pi * sin(pi * x) * cos(pi * x) * sin(pi * y)^2 * cos(t);
p(x, y, t) = cos(pi * x) * cos(pi * y);

%% 非零Dirichlet边界条件
%% 2.1 [0,2]x[-1,1]
ux(x, y, t) = 2 * cos(pi * y) * sin(pi * x) * sin(t);
uy(x, y, t) = -2 * sin(pi * y) * cos(pi * x) * sin(t);
p(x, y, t) = 2 * sin(pi * y) * sin(pi * x) * cos(t);

%% 2.2 [0,1]x[-0.25,0]
ux(x, y, t) = (x^2 * y^2 + exp(-y)) * cos(2 * pi * t);
uy(x, y, t) = (-2 * x * y^3/3 + 2 - pi * sin(pi * x)) * cos(2 * pi * t);
